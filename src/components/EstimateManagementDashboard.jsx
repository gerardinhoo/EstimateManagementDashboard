...
const EstimateManagementDashboard = () => {
  ...
  const aiAnalytics = {
    ...
    analyzeProductivity: (weeklyData) => {
      const avgDaily = weeklyData.reduce((sum, day) => sum + day.count, 0) / 7;
      const trend =
  ...
  const BillingQueue = () => {
    ...
    const saveAmount = (id) => {
      setEstimates((prev) =>
        prev.map((est) =>
          est.id === id ? { ...est, estimateAmount: editValue } : est
        )
      );
    ...
    const markAsBilled = (id) => {
      setEstimates((prev) =>
        prev.map((est) =>
          est.id === id ? { ...est, clientBilled: true } : est
        )
      );
    };
            ...
            {billingEstimates.map((estimate) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() => markAsBilled(estimate.id)}
